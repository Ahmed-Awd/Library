        ...
        <form>
          <div class="row">
                              ...
                              >
                                ...
                                >
                                  <input
                                    ...
                                    @change="updateItemDefault(item.use_default,item)"
                                    true-value="1"
                                    false-value="0"
                                  />
                                  {{ $t("default") }}
            ...
            <div class="col-md-12 mt-4">
              <button class="btn btn-primary" @click.prevent="updateTemplate">
                {{ $t("save") }}
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    listTemplate() {
      axios
        ...
        .then((response) => {
          ...
          console.log(response.data.option_template.optionSecondaries);
        })
        .catch((errors) => {
          console.log(errors);
        });
    ...
    updateItemPrice(e, secondary, primary) {
    ...
    updateItemDefault(value,item){
      item.use_default = value;
      console.log(this.newData)
       let x = this.newData.option_secondaries.filter((el) => {
        return (
          item.id == el.secondary_option_id
          // secondary.id == el.secondary_option_value_id
        );
      });
      if(x){
        x.map(el=>{
          el.use_default = value
        })
      }
    },
          ...
          this.optionsPrimary.map((pEl) => {
            response.data.category.option_values.map((el) => {
              this.newData.option_secondaries.push({
                ...
                use_default: 0,
                // use_default:
                //   response.data.category.optionSecondaries[0]
                //     .secondary_option[0].use_default,
              });
            });
          });
        })
