...
class SimpleXLSX extends Controller
{
    ...
    public function __construct($filename = null, $is_data = null, $debug = null)
    {
        ...
        $this->package = array('filename' => '', 'mtime' => 0, 'size' => 0, 'comment' => '', 'entries' => array());
    ...
    private function _unzip($filename, $is_data = false)
    {
        ...
        $aE = array(0 => $this->_substr($vZ, 0, $pcd), 1 => $this->_substr($vZ, $pcd + 3));
    ...
    public function rowsEx($worksheetIndex = 0)
    {
        ...
        for ($y = 0; $y < $numRows; $y++) {
            for ($x = 0; $x < $numCols; $x++) {
                ...
                $rows[$y][$x] = array(
                    'type' => '', 'name' => $c . ($y + 1), 'value' => '', 'href' => '', 'f' => '', 'format' => '', 'r' => $y
                );
            }
        }
        ...
        foreach ($ws->sheetData->row as $row) {
            ...
            foreach ($row->c as $c) {
                ...
                $rows[$curR][$curC] = array(
                    'type' => $t, 'name' => (string)$c['r'], 'value' => $this->value($c), 'href' => $this->href($c),
                    'f' => (string)$c->f, 'format' => $format, 'r' => $r_idx
                );
                $curC++;
            }
            $curR++;
